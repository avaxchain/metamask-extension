 ...
 * @property {string} provider.type - An identifier for the type of network selected, allows WutangMask to use custom provider strategies for known networks.
 ...
 * @property {MetaMaskState} data - The data emitted from WutangMask controller, or used to initialize it.
 * @property {Number} version - The latest migration version that has been run.
 */

/**
 * Initializes the WutangMask controller, and sets up all platform configuration.
...
async function initialize() {
  ...
  log.debug('WutangMask initialization complete.')
}
 ...
 * @returns {Promise<MetaMaskState>} Last data emitted from previous instance of WutangMask.
 */
async function loadStateFromPersistence() {
  ...
  if (versionedData && !versionedData.data) {
    ...
    sentry.captureMessage('WutangMask - Empty vault found - unable to recover')
  }
  ...
  if (!versionedData) {
    throw new Error('WutangMask - migrator returned undefined')
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('WutangMask - Localstore not supported')
    })
  }
...
/**
 * Initializes the WutangMask Controller with any initial state and default language.
...
function setupController(initState, initLangCode) {
  ...
  pump(
    ...
    (error) => {
      log.error('WutangMask - Persistence pipeline failed', error)
    },
  )
  ...
  async function persistData(state) {
    if (!state) {
      throw new Error('WutangMask - updated state is missing')
    }
    if (!state.data) {
      throw new Error('WutangMask - updated state does not have data')
    }
    if (localStore.isSupported) {
      try {
  ...
  /**
   * Connects a Port to the WutangMask controller via a multiplexed duplex stream.
   * This method identifies trusted (WutangMask) interfaces, and connects them differently from untrusted (web pages).
  ...
  function updateBadge() {
    ...
    extension.browserAction.setBadgeBackgroundColor({ color: '#e84a71' })
  }
...
// On first install, open a new tab with WutangMask
