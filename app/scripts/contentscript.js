...
function injectScript(content) {
  try {
    ...
    console.error('WutangMask: Provider injection failed.', error)
  }
}
...
async function setupStreams() {
  ...
  pump(pageMux, pageStream, pageMux, (err) =>
    logStreamDisconnectWarning('WutangMask Inpage Multiplex', err),
  )
  pump(extensionMux, extensionStream, extensionMux, (err) => {
    logStreamDisconnectWarning('WutangMask Background Multiplex', err)
  ...
  pump(legacyPageMux, legacyPageStream, legacyPageMux, (err) =>
    logStreamDisconnectWarning('WutangMask Legacy Inpage Multiplex', err),
  )
  pump(
    legacyExtensionMux,
    extensionStream,
    ...
    legacyExtensionMux,
    (err) => {
      logStreamDisconnectWarning('WutangMask Background Legacy Multiplex', err)
      notifyInpageOfStreamFailure()
    },
  )
...
function forwardTrafficBetweenMuxes(channelName, muxA, muxB) {
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `WutangMask: Muxed traffic for channel "${channelName}" failed.`,
      error,
    ),
  )
}
...
function forwardNamedTrafficBetweenMuxes(
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `WutangMask: Muxed traffic between channels "${channelAName}" and "${channelBName}" failed.`,
      error,
    ),
  )
}
...
function logStreamDisconnectWarning(remoteLabel, error) {
  console.debug(
    `WutangMask: Content script lost connection to "${remoteLabel}".`,
    error,
  )
}
...
function redirectToPhishingWarning() {
  console.debug('WutangMask: Routing to Phishing Warning component.')
