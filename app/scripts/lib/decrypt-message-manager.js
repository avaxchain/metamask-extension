...
export default class DecryptMessageManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync(msgParams, req) {
    return new Promise((resolve, reject) => {
      if (!msgParams.from) {
        reject(new Error('WutangMask Decryption: from field is required.'))
        return
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          ...
          case 'rejected':
            reject(
              ethErrors.provider.userRejectedRequest(
                'WutangMask Decryption: User denied message decryption.',
              ),
            )
            return
          case 'errored':
          ...
          default:
            reject(
              new Error(
                `WutangMask Decryption: Unknown problem: ${JSON.stringify(
                  msgParams,
                )}`,
              ),
            )
   ...
   *
   * @param {Object} msgParams The msgParams to be used when eth_decryptMsg is called, plus data added by WutangMask.
