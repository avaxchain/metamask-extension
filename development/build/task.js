...
function detectAndRunEntryTask() {
  ...
  if (!taskName) {
    throw new Error(`WutangMask build: No task name specified`)
  }
...
async function runTask(taskName, { skipStats } = {}) {
  if (!(taskName in tasks)) {
    throw new Error(`WutangMask build: Unrecognized task name "${taskName}"`)
  }
  if (!skipStats) {
  ...
  try {
    ...
    console.error(
      `WutangMask build: Encountered an error while running task "${taskName}".`,
    )
    console.error(err)
    process.exit(1)
  }
...
function createTask(taskName, taskFn) {
  if (taskName in tasks) {
    throw new Error(
      `WutangMask build: task "${taskName}" already exists. Refusing to redefine`,
    )
  }
...
function runInChildProcess(task) {
  ...
  if (!taskName) {
    throw new Error(
      `WutangMask build: runInChildProcess unable to identify task name`,
    )
  }
    ...
    await new Promise((resolve, reject) => {
      childProcess.once('close', (errCode) => {
        if (errCode !== 0) {
          reject(
            new Error(
              `WutangMask build: runInChildProcess for task "${taskName}" encountered an error ${errCode}`,
            ),
          )
          return
        }
